...
#include <Config.hpp>  // Rxx keys, WIRE_OHM_PER_M_KEY, defaults
...
public:
    ...
    /**
     * @brief Estimate total bus current (A) from bus voltage and active mask.
     *
     * Uses per-wire resistance values for any enabled outputs in @p mask.
     */
    float estimateCurrentFromVoltage(float busVoltage, uint16_t mask) const;
    ...
    /**
     * @brief Update cached presence/connection state for a wire.
     */
    void setWirePresence(uint8_t index, bool present);
