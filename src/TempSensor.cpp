...
void TempSensor::identifyAndPersistSensors() {
    ...
    // ----- 1) First learning of board sensors (assumes ≥2 present) -----
    if ((b0.length() == 0 || b1.length() == 0) && sensorCount >= 2) {
        ...
        b0 = a; b1 = c;
        DEBUG_PRINTLN("[TempSensor] Learned Board0/Board1 IDs ✅");
    }
    ...
    // (At least 2 sensors will be connected — your guarantee.)
    if (b0.length() == 16 && b1.length() == 16) {
        ...
        if (!b0Present && ucnt > 0) {
            ...
            if (ucnt) --ucnt;
            DEBUG_PRINTLN("[TempSensor] Updated Board0 ID (replacement detected) ♻️");
        }
        if (!b1Present && ucnt > 0) {
            CONF->PutString(TSB1ID_KEY, unknowns[0]);
            b1 = unknowns[0];
            DEBUG_PRINTLN("[TempSensor] Updated Board1 ID (replacement detected) ♻️");
        }
        ...
        if (!b0Present && !b1Present && sensorCount >= 2) {
            ...
            CONF->PutString(TSB1ID_KEY, c);
            b0 = a; b1 = c;
            DEBUG_PRINTLN("[TempSensor] Re-learned Board0/Board1 (both replaced) ♻️");
        }
    }
    ...
    if (b0.length()==16 && b1.length()==16) {
        ...
        if (hs.length()==0) {
            ...
            if (u2 == 1) {
                ...
                hs = unknowns2[0];
                DEBUG_PRINTLN("[TempSensor] Learned Heatsink ID ✅");
            }
        } else {
            ...
            if (!hsPresent && u2 == 1) {
                CONF->PutString(TSHSID_KEY, unknowns2[0]);
                hs = unknowns2[0];
                DEBUG_PRINTLN("[TempSensor] Updated Heatsink ID (replacement) ♻️");
            }
        }
    }
