...
private:
    ...
    IPAddress _sessionIp{0, 0, 0, 0};

    String issueSessionToken_(const IPAddress& ip);
    bool validateSession_(AsyncWebServerRequest* request) const;
    bool sessionIpMatches_(const IPAddress& ip) const;
    void clearSession_();
    ...
    AsyncEventSource eventSse{EP_EVENT_STREAM};
    TaskHandle_t     eventStreamTaskHandle = nullptr;
    uint32_t         eventSeq = 0;
    ...
    AsyncEventSource liveSse{EP_MONITOR_STREAM};
