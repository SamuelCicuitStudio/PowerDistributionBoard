...
Future<void> main() async {
  ...
  runApp(UserWebApp(entryUrl: entryUrl));
}

class UserWebApp extends StatelessWidget {
  const UserWebApp({super.key, required this.entryUrl});
...
class AssetHost {
  static Future<String> prepare() async {
    ...
    final assets = manifest.keys
        .where((key) => key.startsWith('assets/user/'))
        .toList();

    final supportDir = await getApplicationSupportDirectory();
    final baseDir = Directory(
      '${supportDir.path}${Platform.pathSeparator}powerboard_user_assets',
    );
    ...
    for (final asset in assets) {
      final relative = asset.substring('assets/user/'.length);
    ...
    if (!entryFile.existsSync()) {
      throw StateError('Missing user login.html asset.');
    }
