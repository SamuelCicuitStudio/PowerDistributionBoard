...
(function () {
  ...
  function normalizeNtcModel(value) {
    ...
    if (typeof value === "string") {
      ...
      if (v.includes("stein") || v.includes("sh")) return "steinhart";
      return "beta";
    }
    return Number(value) === 1 ? "steinhart" : "beta";
  }
  ...
  function buildLoadControls() {
    return {
      ...
      ntcBeta: state.controls.ntcBeta,
      ntcR0: state.controls.ntcR0,
      ntcFixedRes: state.controls.ntcFixedRes,
      ntcModel: state.controls.ntcModel,
      ntcShA: state.controls.ntcShA,
      ntcShB: state.controls.ntcShB,
      ntcShC: state.controls.ntcShC,
      ntcPressMv: state.controls.ntcPressMv,
      ntcReleaseMv: state.controls.ntcReleaseMv,
      ntcDebounceMs: state.controls.ntcDebounceMs,
      ntcMinC: state.controls.ntcMinC,
      ntcMaxC: state.controls.ntcMaxC,
      ntcSamples: state.controls.ntcSamples,
      ntcGateIndex: state.controls.ntcGateIndex,
  ...
  function updateNtcCal() {
    if (!state.ntc.running) return;
    const tMs = nowMs() - state.ntc.startMs;
    const tempC = synthTemp(tMs, state.ntc.targetC);
    state.ntc.heatsinkC = tempC;
    state.ntc.samples += 1;
    if (tempC >= state.ntc.targetC) {
      state.ntc.running = false;
      state.ntc.done = true;
      stopNtcCalTimer();
      if (state.wireTest.running && state.wireTest.purpose === "ntc_cal") {
  ...
  function startNtcCalTimer() {
    stopNtcCalTimer();
    updateNtcCal();
    state.ntc.timer = setInterval(updateNtcCal, state.ntc.sampleMs);
  }

  function stopNtcCalTimer() {
    if (state.ntc.timer) {
      clearInterval(state.ntc.timer);
      state.ntc.timer = null;
    }
  }
  ...
  function handleCalibStart(body) {
    ...
    const wireIndex = body.wire_index || state.controls.ntcGateIndex || 1;
  ...
  function handleWireTestStart(body) {
    ...
    const idx = body.wire_index || state.controls.ntcGateIndex || 1;
    state.wireTest.running = true;
    state.wireTest.mode = "energy";
  ...
  function handleNtcCalibrate(body) {
    const target = body.target_c || body.ref_temp_c || 100;
    state.ntc.running = true;
    state.ntc.done = false;
    state.ntc.error = "";
    state.ntc.targetC = target;
    state.ntc.samples = 0;
    state.ntc.startMs = nowMs();
    state.ntc.shA = 1.1e-3 + (Math.random() - 0.5) * 1e-4;
    state.ntc.shB = 2.0e-4 + (Math.random() - 0.5) * 1e-5;
    state.ntc.shC = 1.2e-7 + (Math.random() - 0.5) * 1e-8;
    state.wireTest.running = true;
    state.wireTest.mode = "energy";
    state.wireTest.purpose = "ntc_cal";
    state.wireTest.targetC = target;
    state.wireTest.activeWire = state.controls.ntcGateIndex || 1;
    startNtcCalTimer();
    return {
      status: "running",
      target_c: target,
      wire_index: state.wireTest.activeWire,
      sample_ms: state.ntc.sampleMs,
    };
  }

  function handleNtcCalStatus() {
    return {
      running: state.ntc.running,
      done: state.ntc.done,
      error: state.ntc.error,
      target_c: state.ntc.targetC,
      heatsink_c: state.ntc.heatsinkC,
      samples: state.ntc.samples,
      sample_ms: state.ntc.sampleMs,
      sh_a: state.ntc.shA,
      sh_b: state.ntc.shB,
      sh_c: state.ntc.shC,
  ...
  function handleNtcBetaCalibrate(body) {
    const ref = body.ref_temp_c || body.target_c || 25;
    const beta = 3950 + Math.round((Math.random() - 0.5) * 50);
    const r0 = 10000 + Math.round((Math.random() - 0.5) * 300);
    state.controls.ntcBeta = beta;
    state.controls.ntcR0 = r0;
    state.controls.ntcModel = "beta";
    return { status: "ok", ref_temp_c: ref, beta, r0 };
  }
  ...
  function handleControl(body) {
    ...
    if (action === "set") {
      ...
      if (target === "ntcModel") {
        state.controls.ntcModel = normalizeNtcModel(value);
        return { status: "ok", applied: true };
      }
  ...
  async function mockFetch(input, init = {}) {
    ...
    try {
      ...
      // NTC calibrate
      if (path === "/ntc_cal_status") {
        return jsonResponse(handleNtcCalStatus());
      }
      if (path === "/ntc_cal_stop" && method === "POST") {
        state.ntc.running = false;
        state.ntc.done = false;
        state.ntc.error = "stopped";
        stopNtcCalTimer();
        if (state.wireTest.purpose === "ntc_cal") {
          state.wireTest.running = false;
          state.wireTest.purpose = "none";
        }
        return jsonResponse({ status: "stopping" });
      }
      if (path === "/ntc_calibrate" && method === "POST") {
        const body = parseBody(init.body);
        return jsonResponse(handleNtcCalibrate(body));
      }
      if (path === "/ntc_beta_calibrate" && method === "POST") {
        const body = parseBody(init.body);
        return jsonResponse(handleNtcBetaCalibrate(body));
      }
    } catch (err) {
