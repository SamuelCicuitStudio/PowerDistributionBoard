...
void Webview::SendScroll(double delta, bool horizontal) {
  ...
  const auto offset = static_cast<short>(delta * kScrollMultiplier);
  const auto mouse_data = static_cast<UINT32>(static_cast<USHORT>(offset)) << 16;

  // WebView2 routes wheel events based on the mouse position, so make sure we
  // send the wheel event at the last known cursor location within the surface.
  const POINT point = last_cursor_pos_;
  ...
  if (horizontal) {
    ...
    composition_controller_->SendMouseInput(COREWEBVIEW2_MOUSE_EVENT_KIND_WHEEL,
                                            virtual_keys_.state(), mouse_data,
                                            point);
  }
}
