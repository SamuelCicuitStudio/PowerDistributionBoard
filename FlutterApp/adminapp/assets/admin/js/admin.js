(function () {
  ...
  const TOOLTIP_BY_ID = {
    ...
    calibSuggestRefresh:
      "Compute suggested thermal model values from last calibration data.",
    calibSuggestFromHistory:
      "Compute and apply thermal model values from the selected saved history.",
    calibPersistBtn:
      "Persist suggested thermal model values and reload settings.",
  ...
  function bindCalibrationButton() {
    ...
    const suggestFromHistory = document.getElementById(
      "calibSuggestFromHistory"
    );
  ...
  async function saveCalibModelSuggest() {
    ...
    try {
      ...
      openAlert(
        "Calibration",
        "Model values saved. Reloading settings...",
        "success"
      );
      await loadControls();
      fetchCalibModelSuggest();
  ...
  async function computeModelFromHistory() {
    ...
    try {
      ...
      if (!computed) {
        openAlert(
          "Calibration",
          "Not enough data to compute model.",
          "warning"
        );
        return;
      }
  ...
  async function startCalibration(mode) {
    ...
    setCalibText("calibModeText", mode === "model" ? "Model" : "--");
  ...
  async function pollCalibrationOnce() {
    ...
    calibLastMeta = meta;
    const modeLabel = meta.mode === "model" ? "Model" : "--";
