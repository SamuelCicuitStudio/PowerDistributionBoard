...
export function initLiveChart(root = document) {
  const charts = qsa(".live-chart", root);
  if (!charts.length) return;

  charts.forEach((chart) => {
    const scrollWrap = qs("[data-live-scroll]", chart);
    if (!scrollWrap) return;
    const centerBtn = qs("[data-live-center]", chart);
