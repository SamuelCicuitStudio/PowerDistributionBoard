                ...
                ? Listener(
                    ...
                    onPointerSignal: (signal) {
                      if (signal is PointerScrollEvent) {
                        final localPosition = signal.localPosition;
                        final scrollDelta = signal.scrollDelta;
                        final script = '''
(() => {
  try {
    const x = ${localPosition.dx};
    const y = ${localPosition.dy};
    const dx = ${scrollDelta.dx};
    const dy = ${scrollDelta.dy};
    ...
    const isScrollable = (node) => {
      ...
      const canY = (oy === "auto" || oy === "scroll") && node.scrollHeight > node.clientHeight + 1;
      const canX = (ox === "auto" || ox === "scroll") && node.scrollWidth > node.clientWidth + 1;
      return canY || canX;
    };
    ...
    (document.scrollingElement || document.documentElement).scrollBy({ left: dx, top: dy, behavior: "auto" });
    return null;
  } catch (_) {
    return null;
  }
